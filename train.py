from model.unet_model_cbam import UNet
    ...
    with tqdm(total=epochs*per_epoch_num) as pbar:
        for epoch in range(epochs):
            ...
            for image, label in train_loader:
                ...
                # if epoch % 400 == 0:
                #     torch.save(net.state_dict(), 'best_model_' + str(epoch) + '.pth')
                # 更新参数
                loss.backward()
                optimizer.step()
                pbar.update(1)
...
if __name__ == "__main__":
    ...
    print("进度条出现卡着不动是正在计算，请耐心等待。")
    train_net(net, device, data_path, epochs=200, batch_size=2)
